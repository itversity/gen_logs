...
class IPGenerator:
    ...
    def session_gc(self):
        for ip in list(self.sessions.keys()):
            if self.sessions[ip] >= self.session_length:
...
class LogGenerator:
    ...
    def write(self, dest, count):
        for i in range(count):
            ...
            request = quote(request.replace("*PRODUCT*",str(product)).replace("*DEPARTMENT*",dept).replace("*CATEGORY*",cat).lower())
